...
from infinity.utils.diy_tools import self_attn_with_visualize
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, 
                scale_ind=0, block_idx=0, module_idx=0):
        """
        ...
        if self.using_flash:
            ...
            if self.use_flex_attn and attn_fn is not None:
                ...
                # if 4 < scale_ind < 6 : 
                #     scale_size = scale_schedule[scale_ind][-1]
                #     oup = self_attn_with_visualize(q, k, v, scale=self.scale, 
                #                                    save_path=f"outputs/attn_maps/attn_map_sclae{scale_ind}block{block_idx}module{module_idx}.png",
                #                                    if_tok=False, use_log=True,scale_size= scale_size)  # visualize attention map
                # elif scale_ind >=10:
                #     scale_size = scale_schedule[scale_ind][-1]
                #     oup = self_attn_with_visualize(q, k, v, scale=self.scale, 
                #                                    save_path=None,
                #                                    if_tok=False, use_log=True,scale_size= scale_size)  # visualize attention map            # oup: bf16
                # else:
                #     oup = slow_attn(query=q, key=k, value=v, scale=self.scale, attn_mask=attn_bias_or_two_vector, dropout_p=0).transpose(1, 2).reshape(B, L, C)
...
class CrossAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, block_idx=0, module_idx=0):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            if self.checkpointing_sa_only and self.training:
                ...
                x_sa = self.sa(x_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, 
                               scale_ind=scale_ind, block_idx=block_idx, module_idx=module_idx)
            x = x + self.drop_path(x_sa.mul_(gamma1))
            x = x + self.ca(self.ca_norm(x), ca_kv).float().mul_(self.ca_gamma)
