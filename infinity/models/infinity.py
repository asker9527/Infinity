...
from infinity.utils.diy_tools import decoder_img
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        # 逻辑: 不像 GPT 那样逐个 Token 预测，而是逐层 Scale (si) 预测整个 Token Map (pn)。
        # ==============================================================================
        for si, pn in enumerate(scale_schedule):   # si: 当前尺度索引 k, pn: 当前尺度分辨率 h_k x w_k
            cfg = cfg_list[si]
            if si >= trunk_scale: # 截断生成（Debug 或生成低分辨率预览）
                break
            cur_L += np.array(pn).prod() # 统计当前累计的 Token 数量

            # [Engineering] Flex Attention 相关的 Padding 逻辑，用于硬件加速
            need_to_pad = 0
            attn_fn = None
            if self.use_flex_attn:
            ...
            # ==============================================================================
            # PART 2: VAR Transformer Inference (VAR Transformer 推理)
            # 对应论文: "VAR transformer... takes ([s], r1...rk-1) as input to predict rk"
            # 逻辑: 计算 p(r_k | r_1, ..., r_{k-1})。last_stage 承载了前序尺度的信息（Prefix）。
            # ==============================================================================
            layer_idx = 0
            for block_idx, b in enumerate(self.block_chunks):
                # [Position Embedding] 注入位置编码/尺度编码
                # 确保模型知道当前是在预测哪一个 Scale
                if self.add_lvl_embeding_only_first_block and block_idx == 0:   
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=need_to_pad)
                if not self.add_lvl_embeding_only_first_block: 
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=need_to_pad)
                
                # [Transformer Block] 核心注意力机制与前馈网络
                for module_idx, m in enumerate(b.module):
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, 
                                   attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, 
                                   rope2d_freqs_grid=self.rope2d_freqs_grid, 
                                   scale_ind=si, block_idx=block_idx, module_idx=module_idx 
                                   )
                    
                    # [CFG] Layer-wise Classifier-Free Guidance (层间 CFG)
                    # 这是一个高级技巧：在 Transformer 中间层特征上混合 Conditional 和 Unconditional 的流
            ...
            # ==============================================================================
            # PART 3: Logits Calculation & Sampling (Logits 计算与采样)
            # 对应论文: "predict (r1, r2, ... rK)" via parallel token generation
            # 逻辑: 将 Transformer 的隐层输出映射为词表大小的 Logits，并采样出离散 Token。
            # ==============================================================================
            if (cfg != 1) and add_cfg_on_logits:
                # 标准 CFG 公式：Uncond + s * (Cond - Uncond)
                logits_BlV = self.get_logits(last_stage, cond_BD).mul(1/tau_list[si])
                logits_BlV = cfg * logits_BlV[:B] + (1-cfg) * logits_BlV[B:]
            else:
                logits_BlV = self.get_logits(last_stage[:B], cond_BD[:B]).mul(1/tau_list[si])
            
            # [Sampling] Top-k / Top-p 采样，获取离散索引 idx_Bl (即论文中的 r_k)
            if self.use_bit_label: # 特殊的 bit labeling 逻辑
            ...
            # ==============================================================================
            # PART 4: Residual Accumulation & Up-interpolation (残差累加与上采样)
            # 对应论文: "Residual design" & "Up-interpolation" (Algo 2 in Paper)
            # 逻辑: VAR 的核心。当前预测的 Token 是"残差"。我们需要将其解码为向量，
            # 累加到之前的图像特征上，然后上采样作为下一轮的输入。
            # ==============================================================================
            if vae_type != 0:
                assert returns_vemb
                # [Ground Truth Leaking] 调试或引导用：前几层强制使用真实 Token
                if si < gt_leak:
                    idx_Bld = gt_ls_Bl[si]
                else:
                    assert pn[0] == 1
                    # Reshape 这里的 Token 序列为 2D 图像形状 [B, h, w, d]
                    idx_Bld = idx_Bld.reshape(B, pn[1], pn[2], -1) 
                    
                    # [Spatial Patchify] 空间 Patch 化操作 (Unpatchify)
                    # 将通道深度转换为空间分辨率，用于处理高分辨率生成的工程优化
                    if self.apply_spatial_patchify: 
                ...
                # [Quantization Decode] 将离散 Index 映射回 VQVAE 的 Codebook 向量 (z_k)
                codes = vae.quantizer.lfq.indices_to_codes(idx_Bld, label_type='bit_label') 
                
                if si != num_stages_minus_1: # 如果不是最后一层
                    # 1. 累加残差：summed_codes 是当前所有尺度的叠加结果 (f_hat)
                    # 注意：这里会先插值到最大分辨率进行累加，这是 Residual Design 的具体实现
                    summed_codes += F.interpolate(codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                    
                    # 2. 准备下一轮输入 (Up-interpolation)
                    # 将累加好的特征上采样到 下一层 (si+1) 的分辨率，作为 Transformer 的输入
            ...
            # [Next Scale Prep] 为下一轮 Transformer 准备 Embedding
            # 将 VQVAE 的特征空间映射回 Transformer 的 Hidden Dim
            if si != num_stages_minus_1:
                last_stage = self.word_embed(self.norm0_ve(last_stage))
                last_stage = last_stage.repeat(bs//B, 1, 1) # 复制以支持 CFG (Batch size doubling)

            # decoder_img(vae, summed_codes, f"./outputs/debug_infinity_decoded_pn{pn[-1]}.png")

        # ==============================================================================
        # PART 5: Final Image Reconstruction (最终图像重建)
        # 对应论文: "Reconstructed image im_hat = D(f_hat)"
        # 逻辑: 循环结束，使用 VQVAE 的 Decoder 将 Latent 特征转换为像素图像。
        # ==============================================================================
        if inference_mode:
            # 清理 KV Cache
